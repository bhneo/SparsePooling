...
def scaled_dot_product_attention(q, k, v, mask, causality=True):
    ...
    paddings = tf.ones_like(scaled_att_logits) * (-2 ** 32 + 1)  # 把padding的权重改成无限小
    ...
    return outputs  # (None, seq_len, d_model)
...
class EncoderLayer(Layer):
    def __init__(self, d_model, num_heads=1, ffn_hidden_unit=64, dropout=0.2, use_norm=True, causality=True):
        """
        Encoder Layer
        :param d_model: A scalar. The self-attention hidden size.
        ...
        :param use_norm: Boolean. If True, using layer normalization, default True
        :param causality: Boolean. If True, using causality, default True
        """
        ...
        self.layernorm1 = LayerNormalization(epsilon=1e-6, trainable=use_norm)
        self.layernorm2 = LayerNormalization(epsilon=1e-6, trainable=use_norm)
    ...
    def call(self, inputs):
        x, mask = inputs
        # self-attention
        att_out = self.mha(x, x, x, mask)  # run call（None, seq_len, d_model)
