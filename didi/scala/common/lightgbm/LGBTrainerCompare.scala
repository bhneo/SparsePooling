...
/**
  ...
  */
object LGBTrainerCompare extends lib.MainSparkApp with IColorText {
  ...
  def train(trainDF: DataFrame): LightGBMClassificationModel = {
    ...
    /******************  model binary  ********************/
    model.write.overwrite().save(s"${param.output_model}/binary")

    /******************  model txt  ********************/
    model.saveNativeModel(s"${param.output_model}/txt",true)
    ToolsHdfs.getHdfsFile(
      log,
      s"${param.output_model}/binary",
      s"${param.path_local_model}"
    )
    ToolsHdfs.getHdfsFile(
      log,
      s"${param.output_model}/txt",
      s"${param.path_local_model}"
    )
