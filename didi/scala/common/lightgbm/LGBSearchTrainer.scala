...
/**
  ...
  */
object LGBSearchTrainer extends lib.MainSparkApp with IColorText {
  ...
  def search(): Unit = {
    ...
    /******************  model binary  ********************/
    bestModel.write.overwrite().save(s"${param.best_model}/binary")

    /******************  model txt  ********************/
    bestModel.saveNativeModel(s"${param.best_model}/txt",true)
    ToolsHdfs.getHdfsFile(
      log,
      s"${param.best_model}/binary",
      s"${param.path_local_model}"
    )
    ToolsHdfs.getHdfsFile(
      log,
      s"${param.best_model}/txt",
      s"${param.path_local_model}"
    )
