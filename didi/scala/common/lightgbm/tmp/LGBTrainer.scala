...
import com.cxyx.common.lightgbm.Utils
...
object LGBTrainer extends lib.MainSparkApp with IColorText {
  ...
  def train(trainDF: DataFrame): LightGBMRankerModel = {
    val lgboost = new LightGBMRanker()
      .setFeaturesCol("features")
      .setLabelCol("label")
      .setObjective("lambdarank")
      .setBoostingType("gbdt")
      .setGroupCol("request_id")
      .setMetric("ndcg")
  ...
  def evaluate(model: LightGBMRankerModel,
               trainDF: DataFrame,
               testDF: DataFrame
              ) = {
  ...
  def getDataSet: DataFrame = {
    ...
    if (ToolsHdfs.checkHdfsFileExists(log, cache)) {
      ...
      originData.saveCompressedParquet(cache)
